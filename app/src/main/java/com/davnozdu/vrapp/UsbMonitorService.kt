...
class UsbMonitorService : Service() {
    ...
    private suspend fun evaluate(reason: String) {
        ...
        when {
            present && !isConnected -> {
                // В режиме гарнитуры дисплей включён нарочно — иначе очки не
                // поднимут аудиоусилитель и звука не будет совсем. Картинки
                // там нет, поэтому гасить экран телефона незачем.
                if (RootUtils.headsetMode()) {
                    log("Режим гарнитуры — экран телефона не гашу")
                    return
                }
